...
In doing so we whish to achieve a more modular and standerdized
coding format.
Note: More information is located in each folder in refrence 
to each Sub System)

We have also started to use a sigle instance of commonly called 
...
Commets/Notes: if code is getting complicated enough where a quick
look over the code does not explain what it does please add 
Commets/Notes using: //(Commet/Note)
Note: These files do note automatically go to the next line 
so please go to the next line if the Commet/Note is reaching 
this size with no end soon.
